class Priority < ActiveRecord::Base
  ...
  acts_as_set_sub_instance :table_name=>"priorities"

  if Instance.current and Instance.current.is_suppress_empty_priorities?
  ...
  def endorse(user,request=nil,sub_instance=nil,referral=nil)
    ...
    if not endorsement
      endorsement = Endorsement.new(:value => 1, :priority => self, :user => user, :sub_instance => sub_instance, :referral => referral)
 ...
 #   self.change = nil    
  ...
  def show_url
    if self.sub_instance_id
      Instance.current.homepage_url(self.sub_instance) + 'priorities/' + to_param
    else
      Instance.current.homepage_url + 'priorities/' + to_param
    end
  end
